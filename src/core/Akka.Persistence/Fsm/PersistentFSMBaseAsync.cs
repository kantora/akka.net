...
namespace Akka.Persistence.Fsm
{
    ...
    public abstract class PersistentFSMBaseAsync<TState, TData, TEvent> : PersistentActor, IListeners
    {
        public delegate Task<State<TState, TData, TEvent>> StateFunction(
        ...
        protected PersistentFSMBaseAsync()
        {
        ...
        protected async Task ProcessMsg(object any, object source)
        {
            var fsmEvent = new FSMBase.Event<TData>(any, _currentState.StateData);
            ProcessEvent(fsmEvent, source);
        }

        private async Task ProcessEvent(FSMBase.Event<TData> fsmEvent, object source)
        {
            if (DebugEvent)
            {
            ...
            if (stateFunc != null)
            {
                upcomingState = await stateFunc(fsmEvent);
            }

            if (upcomingState == null)
            {
                upcomingState = await HandleEvent(fsmEvent);
            }
        ...
        {
            ActorTaskScheduler.RunTask(() => ReceiveCommandAsync(message));
            return true;
        }

        protected async Task ReceiveCommandAsync(object message)
        {
            message.Match()
                .With<TimeoutMarker>(async marker =>
                {
                    if (_generation == marker.Generation)
                    {
                        await ProcessMsg(new StateTimeout(), "state timeout");
                    }
                })
                .With<Timer>(async t =>
                {
        ...
        public sealed class TransformHelper
        {
            ...
            public StateFunction Using(Func<State<TState, TData, TEvent>, Task<State<TState, TData, TEvent>>> andThen)
            {
                StateFunction continuedDelegate = async (@event, state) => await andThen.Invoke(await Func(@event, state));
